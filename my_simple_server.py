...
class AsyncWebServer:
    ...
    async def handle_procedure(self, request_line, writer=None):
        request_json = None
        ...
        if procedure == "get_all_afe_configuration":
            # 1. Start the root JSON object
            await writer.awrite(b"{")
            
            first_device = True
            for afe_device in self.hub.afe_devices:
                # 2. Add commas between elements, but not before the first one
                if not first_device:
                    await writer.awrite(b",")
                first_device = False
                
                # 3. Format the key string without f-strings: "device_id":
                key_str = '"' + str(afe_device.device_id) + '":'
                await writer.awrite(key_str.encode())
                
                # 4. Serialize and write ONLY this single device's configuration
                value_bytes = ujson.dumps(afe_device.configuration).encode()
    ...
    async def handle_client(self, reader, writer):
        try:
            ...
            await p.print("Connection from", peername)
            self.tcp_requests[peername] = 1
            # 1. Check server capacity
            if len(self.tcp_requests) >= self.tcp_requests_max:
                await p.print("!!!!!!!!!!!!", self.RESPONSE_SERVER_BUSY)
                await writer.awrite(self.RESPONSE_SERVER_BUSY)
                return

            # 2. Read with a 5-second timeout
            request_line = await uasyncio.wait_for(reader.readline(), 5)
            await p.print("#", request_line)
            if request_line.startswith(b"GET"):
                # Read and discard remaining headers until an empty line is found
                while True:
                    line = await uasyncio.wait_for(reader.readline(), 2)
                    if line == b"\r\n" or line == b"\n" or not line:
                        break
            ...
            await p.print("Error handling client", peername,":", e)
        finally:
            self.tcp_requests.pop(peername, None)
        #     await p.print("EXIT")
            try:
                writer.close() # Standard close
                await writer.wait_closed() # Wait for socket to fully release
            except Exception:
                pass


    async def handle_clientx(self, reader, writer):
        peername = writer.get_extra_info('peername')
        try:
            request_line = await uasyncio.wait_for(reader.readline(), 5)
            if not request_line: # Client closed connection before sending anything
                ...
                return
            await p.print("Its peername:",peername,"for request", request_line)
            ...
            if response:
                ...
                while True:
                    ...
                    if header_line == b"\r\n" or not header_line: # Empty line signifies end of headers or client closed
                        break
                await p.print("Its header line:", header_line)
                await self.send_control_web_page(writer)
        except OSError as e:
